...
from src.random_forest import (
    ...
    load_random_forest_manifest,
    normalize_candidate_key,
    save_random_forest_manifest,
    score_saved_model,
    select_candidate,
)
...
def test_build_random_forest_estimator_fits_multioutput_and_is_reproducible() -> None:
    ...
    first = build_random_forest_estimator(
        n_estimators=12,
        max_depth=8,
        min_samples_leaf=2,
        max_features="sqrt",
    ...
    second = build_random_forest_estimator(
        n_estimators=12,
        max_depth=8,
        min_samples_leaf=2,
        max_features="sqrt",
    ...
    assert isinstance(first, RandomForestRegressor)
...
def test_build_random_forest_estimator_rejects_invalid_parameters(
    kwargs: dict[str, object],
) -> None:
    with pytest.raises((TypeError, ValueError)):
        build_random_forest_estimator(**kwargs)  # type: ignore[arg-type]
...
def test_random_forest_manifest_round_trips_nullable_parameters(tmp_path: Path) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    manifest_path = tmp_path / "random_forest.json"
    _save(dataset, manifest_path)

    manifest = load_random_forest_manifest(
        manifest_path,
        contract=dataset.contract,
        feature_subsets=dataset.feature_subsets,
    )
...
def test_load_random_forest_manifest_reports_missing_manifest(tmp_path: Path) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    with pytest.raises(FileNotFoundError, match="Run 04_05_train_random_forest"):
        load_random_forest_manifest(
...
def test_load_random_forest_manifest_rejects_an_older_schema_version(
    tmp_path: Path,
) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    manifest_path = tmp_path / "random_forest.json"
...
def test_save_random_forest_manifest_rejects_invalid_test_records(
    tmp_path: Path,
) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    manifest_path = tmp_path / "random_forest.json"
...
def test_save_random_forest_manifest_rejects_unknown_selected_subset(
    tmp_path: Path,
) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    with pytest.raises(ValueError, match="is not a known subset"):
        _save(dataset, tmp_path / "random_forest.json", selected_subset="unknown")


def test_load_random_forest_manifest_rejects_invalid_bootstrap_and_random_state(
    tmp_path: Path,
) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    manifest_path = tmp_path / "random_forest.json"
...
def test_random_forest_manifest_rejects_contract_mismatches(tmp_path: Path) -> None:
    dataset = _write_feature_artifacts(tmp_path)
    manifest_path = tmp_path / "random_forest.json"
    _save(dataset, manifest_path)
    stored = json.loads(manifest_path.read_text(encoding="utf-8"))
...
def test_score_saved_model_uses_manifest_feature_order(
    ...
    fake_model = _FakeSavedRandomForest()
    monkeypatch.setattr(random_forest, "load_joblib", lambda _path: fake_model)
